    ...
    vector<vector<int>> init_filter()
    {
        ...
        for (int i = 0; i < filter_size; ++i)
        {
            for (int j = 0; j < filter_size; ++j)
            {
                ...
                {
                    res[i][j] = filter_size / 2 - (center_val - (abs(i - center_x) + abs(j - center_y)));
                }
            }
        }
        return res;
...
int main()
{
    ...
    char filename[128];
    snprintf(filename, sizeof(filename), "result_distinc_n_%d_s_%.2f_r_%.2f_steps_%d_randseed_%.csv",
