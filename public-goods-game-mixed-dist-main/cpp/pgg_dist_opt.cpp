...
private:
    ...
    void set_neighbors(vector<vector<int>> &buf, int x, int y) {
        buf[0][0] = x; buf[0][1] = y;
        buf[1][0] = (x-1+n)%n; buf[1][1] = y;
        buf[2][0] = (x+1)%n; buf[2][1] = y;
        buf[3][0] = x; buf[3][1] = (y-1+n)%n;
        buf[4][0] = x; buf[4][1] = (y+1)%n;
    }
...
int main() {
    ...
    char filename[128];
    snprintf(filename, sizeof(filename), "result_dist_n_%d_s_%.2f_u_%.2f_steps_%d_randseed_%d.csv",
        n, s, u, steps, RANDOM_SEED);
